...
void BungeeCordServer::backupServer()
{
	...
	*serverProcess << "say SERVER BACKUP STARTING. BungeeCordServer going readonly..." << std::endl;
	...
	*serverProcess << "say SERVER BACKUP ENDED. BungeeCordServer going read-write..." << std::endl;
...
void BungeeCordServer::stopServer()
{
	...
	if (serverProcess != nullptr)
	{
		...
		*serverProcess << "stop" << std::endl;
		...
		//~ delete(serverProcess);
		serverProcess = NULL;
		log->info("BungeeCordServer stopped");
	} else {
		log->info("BungeeCordServer already stopped");
	}
}
...
//~ std::string BungeeCordServer::getOutput(int timeout, int lines)
//~ {
    ...
    //~ return _output.str();
//~ }
void BungeeCordServer::logger(size_t linesRequested, std::stringstream* output, log4cpp::Category* log)
{
	std::string _output;
